...
import { BTS_ORDER, CATEGORY_ORDER, usePaletteOrDark } from "@/lib/palette";
import type {
    ...
    >
      <ResponsiveContainer>
        <BarChart data={data} barCategoryGap="22%" margin={{ top: 4, right: 8, left: -12, bottom: 0 }}>
          ...
          <Tooltip {...tooltipStyle(P)} formatter={(v) => [`${v} min`]} />
          <Legend wrapperStyle={{ fontSize: 11.5 }} />
          {BTS_ORDER.map((airport) => (
            <Bar key={airport} dataKey={airport} fill={P.airport[airport]} radius={[3, 3, 0, 0]} />
          ))}
        </BarChart>
      </ResponsiveContainer>
    </ChartCard>
    ...
    >
      <ResponsiveContainer>
        <AreaChart data={q.data?.years ?? []} margin={{ top: 4, right: 8, left: -14, bottom: 0 }}>
          ...
          <Legend wrapperStyle={{ fontSize: 11.5 }} />
          <Area dataKey="nas_khr" name="NAS" stroke={P.causes.nas} fill={P.causes.nas + "35"} strokeWidth={2} />
          <Area dataKey="weather_khr" name="weather" stroke={P.causes.weather} fill={P.causes.weather + "35"} strokeWidth={2} />
        </AreaChart>
      </ResponsiveContainer>
    </ChartCard>
  );
    ...
    >
      <ResponsiveContainer>
        <BarChart data={data} margin={{ top: 4, right: 8, left: -8, bottom: 0 }}>
          ...
          <Legend wrapperStyle={{ fontSize: 11.5 }} />
          {BTS_ORDER.map((airport) => (
            <Bar key={airport} dataKey={airport} stackId="s" fill={P.airport[airport]} />
          ))}
        </BarChart>
      </ResponsiveContainer>
    </ChartCard>
              ...
              <tr key={i} className="border-t border-border/60">
                ...
                <td className="py-1.5 pr-3">
                  <span className="flex items-center gap-1.5">
                    <span className="h-1.5 w-1.5 rounded-full" style={{ background: P.airport[d.airport] }} />
                    {d.airport}
                  </span>
                </td>
                <td className="py-1.5 pr-3">
                  <span style={{ color: d.worst_category ? P.category[d.worst_category] : P.neutral }}>
                    {d.worst_category ?? "—"}
                  </span>
                </td>
    ...
    >
      <ResponsiveContainer>
        <LineChart data={data} margin={{ top: 4, right: 8, left: -14, bottom: 0 }}>
          ...
          />
          <YAxis width={44} tickFormatter={(v) => String(Math.round(Number(v)))} />
          <Tooltip {...tooltipStyle(P)} labelFormatter={(v) => String(v).slice(0, 7)} />
          <Legend wrapperStyle={{ fontSize: 11.5 }} />
          {CATEGORY_ORDER.map((cat) => (
            <Line
              key={cat} dataKey={cat} stroke={P.category[cat]} dot={false}
